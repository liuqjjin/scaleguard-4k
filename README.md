...
阈值不能在评估集上调。标定流程：人工对每个带指标的尺度步打 `acceptable` 标签 → 只用非 mock 的可接受样本 → 以**输入图像 SHA-256 聚类**为重采样单元（不是单个递归步，也不是重复种子）→ 质量取 0.05 下分位、误差取 0.95 上分位 → 2000 次 bootstrap、95% 区间、种子 20250727 → 至少 20 个可接受聚类，不足则写 `insufficient_data` 并非零退出。
...
分位标定定义的是"可接受样本包络"，它不证明最优分类、因果性或人类偏好的泛化。需要报告对分位、最小样本数、种子、数据集与标注者一致性的敏感性。
